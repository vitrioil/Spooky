...
def preprocess(location="../input/train.csv", itr=None, tokenized=False):
	...
	def generate_pairs(word_distance, batch_size, itr=None):
		''' 
		itr should be in tokenized form
		'''
		if itr is None:
			itr = df["text"]
		context,target = [],[]
	...
	with tf.Session() as sess:
		try:
			...
			for e in range(epochs):	
				epoch_loss = 0
				epoch_acc = 0
				for indx,pair in enumerate(generate_pairs(word_distance, batch_size, itr=document)):
					context, target = pair
					_, c, a  = sess.run(
...
#	train(word_distance=5, batch_size=128, lr=1e-4)
